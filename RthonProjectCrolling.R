...
# 부채비율 저장하기 위한 데이터 프레임

i<-1

n<-nrow(x=codes) # nrow를 이용하서 개수 설정


for(i in 1:n){  //  n 개 수 만큼 for문 반복

  cat(str_glue('현재 {i}번째 회사 정보 수집중 !'),'\n')
  res<-GET(url='https://finance.naver.com/item/main.nhn',
           query = list('code'=codes$code[i]))
  # naver에 있는 자료를 크롤링 하여 가져오려고 한다.

  tryCatch(expr={  # tryCatch() = 에러가 날 경우의 예외처리를 위한 tryCatch()함수
    tbl <- res %>%
      read_html(encoding = 'EUC-KR')%>%   # 인코딩(EUC-KR)
      html_node(css ='#content > div.section.cop_analysis > div.sub_section >
            table')%>%
      html_table(fill = TRUE) # 원소가 모자라거나 불러오지 못할 때
    # 원소들은 전부 ‘NA’로 결측값 처리

    colnames(x=tbl)[1:1] <- str_c('total')  # 1:1 행에 열 이름을 total로 변경, 한글 사용X
    colnames(x=tbl)[2:5] <- str_c('실적',2017:2020) # 2:5행부터 실적 값들을 불러온다.

    df2<-
      tbl %>%
      select(1:5)%>%
      slice(c(3))%>%
      mutate(회사 = codes$name[i])	# 새로운 파생 column을 만드는 함수

    results <-rbind(results,df2)	#results, df2를 행 경합 하여 result에 저장

    Sys.sleep(time = 0.5L) # 어떻게 돌릴지 지정
  }, error = function(e) cat('---> 에러 발생!\n'))
}		#에러가 발생하게 되면 에러 메시지 출력과 동시에 다음 값으로 넘어간다.
...
results$실적2017 <- as.integer(x = results$실적2017)# integer 타입 정수로 변경
results$실적2018 <- as.integer(x = results$실적2018)# integer 타입 정수로 변경
results$실적2019 <- as.integer(x = results$실적2019)# integer 타입 정수로 변경
results$실적2020 <- as.integer(x = results$실적2020)# integer 타입 정수로 변경

#dplyr
results <- results %>% select(-total)
